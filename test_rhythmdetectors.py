...
# need test dataframe test_hr1 that has 3 columns and 3 rows, where each
# successive row will be tagged as no arrythmia, bradycardia, and tachycardia
...
def get_test_hr1(a=60, b=60, c=60):
    ...
    """
    initial_messages = ['Healthy... for now',
                        'Healthy... for now',
                        'Healthy... for now']
    test_hr1 = pd.DataFrame({'B/T': initial_messages,
                             'time': [0, 5, 10],
                             'HeartRate': [a, b, c]})
    return test_hr1
...
def test_detect_rhythm_brady():
    ...
    output_messages = ['Bradycardia Detected',
                       'Healthy... for now',
                       'Healthy... for now']
    output_hr = {'B/T': output_messages,
                 'time': [0, 5, 10],
                 'HeartRate': [20, 60, 60]}
...
def test_detect_rhythm_brady2():
    ...
    output_messages = ['Bradycardia Detected',
                       ...
                       'Bradycardia Detected']
    output_hr = {'B/T': output_messages, 'time': [0, 5, 10],
                 'HeartRate': [60, 60, 60]}
...
def test_detect_rhythm_brady3():
    ...
    output_messages = ['Healthy... for now',
                       'Healthy... for now',
                       'Healthy... for now']
    output_hr = {'B/T': output_messages,
                 'time': [0, 5, 10],
...
def test_detect_rhythm_tachy3():
    ...
    output_messages = ['Tachycardia Detected',
                       'Tachycardia Detected',
                       'Tachycardia Detected']
    output_hr = {'B/T': output_messages,
                 'time': [0, 5, 10],
...
def test_detect_rhythm_bradytachy():
    ...
    Test output of tachy/bradydetector when brady threshold is set to 50
    and tachy to 100.
    """
    bm_bt = ECGMeasure()
