...
def thresholdhr(hr_rawdata):
    ...
    :return: will return a dataframe of numbers for thresholding
    whether a heart beat has occured
    """
    ...
    for j in range(0, number_chunks):
        ...
        maxhr_data = max(hr_rawdata['voltage'][
...
def hrdetector(hr_rawdata):
    ...
    and estimate both instanteous and hr over delta_t
    ...
    for j in range(0, len(thresholds)):
        for i in range(1, data_chunk):
            if (hr_rawdata['voltage'][i * (j + 1)] > thresholds['Threshold'][
                j]) and \
                    (hr_rawdata['voltage'][(i * (j + 1)) - 1] <
...
def bradydetector(hr, hr_threshold_brady):
    ...
    :returns: Number of instances of Bradycardia, time stamp for Bradycardia,
    return HR list with added colomns
    """
...
def tachydetector(hr, hr_threshold_tachy):
    ...
    :return: Number of instances of Bradycardia, time stamp for Bradycardia,
    return HR list with added colomns
    """
