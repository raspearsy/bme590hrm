...
def input_dataframe(file="ecg_data.csv"):
    ...
    :param file: A CSV file containing ECG Data with time (s)
    in the first column and voltage (mV) in the second column.
    :return: An DataFrame for the time and voltage ecg data
    """

    try:
        ecg_dataframe = pd.read_csv(file, names=['time', 'voltage'],
                                    skiprows=1)
