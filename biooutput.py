    ...
    def summarize_ecg(self):
        ...
        with open('ecgOutput.txt', 'w') as ecgResults:
            instHRStartstr = "Estimated instantaneous heart rate at {}s: {}BPM" \
                .format(startTime, self.data['HeartRate'][startInd])
            instHRStopstr = "Estimated instaneous heart rate at {}s: {}BPM".format(stopTime, self.data['HeartRate'][stopInd])
            avgHR = self.data['HeartRate'][startInd:stopInd].mean()
            ...
            for j in range(startInd, stopInd):
                if self.data['B/T'][j] == 'Bradycardia Detected':
                    bradyTimes.append(self.data['time'][j])
                elif self.data['B/T'][j] == 'Tachycardia Detected':
                    tachyTimes.append(self.data['time'][j])
